...
import { ChangeDetectorRef, Component, Input, OnInit, ViewChild, AfterViewInit, Inject, HostListener, ElementRef } from '@angular/core';
...
@ViewChild (ModalHistoricoComponent) modalHistorico: ModalHistoricoComponent
  ...
  displayedColumns: string[] = ['usuario', 'data', 'titulo', 'conteudo', 'acao'];
  ...
  obterDadosIniciais() {
      this.dashboardService.ListarArtigosEdicoesHistorico('1', '10').subscribe(response => {
  ...
  obterProximosDados(filtro: string) {
    ...
    this.dashboardService.ListarArtigosEdicoesHistoricoComFiltro(this.paginaAtual.toString(), this.tamanhoDaPaginaAtual.toString(), filtroBusca.trim()).subscribe(response => {
  ...
  incializarTebela() {
    ...
    this.dataSource = new MatTableDataSource<ArtigoEdicaoDataTableDto>(this.listaTemasArtigos);
  ...
  acaoVisualizarEdicao(dado){
    ...
    this.dashboardService.sessionStorageSalvarUltimoId(this.artigoEdicao.artigoId)
    this.router.navigate([`/wiki/historico-detalhe-edicao/${dado.edicaoId}`]).then();
  }
